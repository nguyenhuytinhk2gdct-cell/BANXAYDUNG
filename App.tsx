...
import { Send, Upload, Trash2, FileText, Loader2, Database, X, ChevronRight, Users, MessageSquare, Filter, BrainCircuit, Sparkles, FileCheck, ScrollText, Search, ListFilter, BookmarkCheck } from 'lucide-react';
...
const KNOWLEDGE_BASE_3 = {
  "id": "KB3",
  "title": "Văn bản Chỉ thị, Nghị quyết",
  "docs": [
    "Chỉ thị 35-CT/TW - Đại hội Đảng bộ các cấp",
    "Nghị quyết Hội nghị lần thứ 10 Ban Chấp hành TW khóa XIII",
    "Chỉ thị về tăng cường sự lãnh đạo đối với chuyển đổi số",
    "Nghị quyết Trung ương về xây dựng, chỉnh đốn Đảng"
  ]
};
...
const KEYWORDS_KB3 = ["chỉ thị 35", "nghị quyết", "nq/tw", "ct/tw", "hướng dẫn số", "kết luận số", "văn bản pháp quy"];

const DOC_TYPES = [
  { label: "Tất cả", value: "all" },
  { label: "Chỉ thị", value: "Chỉ thị" },
  { label: "Nghị quyết", value: "Nghị quyết" },
  { label: "Kết luận", value: "Kết luận" },
  { label: "Hướng dẫn", value: "Hướng dẫn" },
  { label: "Quyết định/Quy định", value: "Quy" }
];
...
const App: React.FC = () => {
  ...
  const [showDocClassification, setShowDocClassification] = useState(false);
  const [pendingFile, setPendingFile] = useState<{ name: string; mimeType: string; data: string } | null>(null);
  const [isPro, setIsPro] = useState(false);
  
  // States for classification filtering
  ...
  const allDocuments = useMemo(() => {
    const staticDocs = [
      ...KNOWLEDGE_BASE_1.docs.map(d => ({ name: d, category: 'KB1' as const })),
      ...KNOWLEDGE_BASE_2.docs.map(d => ({ name: d, category: 'KB2' as const })),
      ...KNOWLEDGE_BASE_3.docs.map(d => ({ name: d, category: 'KB3' as const }))
    ];
    const uploadedDocs = knowledgeFiles.map(f => ({ name: f.name, category: f.category }));
  ...
  const filteredDocuments = useMemo(() => {
    return allDocuments.filter(doc => {
      ...
      const matchesType = selectedType === "all" || doc.name.toLowerCase().includes(selectedType.toLowerCase());
      return matchesSearch && matchesType;
    });
  }, [allDocuments, searchQuery, selectedType]);

  const classifyFile = (name: string): 'KB1' | 'KB2' | 'KB3' | null => {
    const lowerName = name.toLowerCase();
    if (KEYWORDS_KB3.some(kw => lowerName.includes(kw))) return 'KB3';
    if (KEYWORDS_KB1.some(kw => lowerName.includes(kw))) return 'KB1';
    if (KEYWORDS_KB2.some(kw => lowerName.includes(kw))) return 'KB2';
    return null;
  };
  ...
  const finalizeClassification = (category: 'KB1' | 'KB2' | 'KB3') => {
    if (pendingFile) {
  ...
  return (
    <div className="flex flex-col h-screen bg-[#F8F9FA] relative overflow-hidden text-slate-800">
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[200] flex items-center justify-center p-4 animate-fadeIn">
          <div className="bg-white rounded-[32px] shadow-2xl max-w-md w-full overflow-hidden border border-slate-100 animate-scaleIn">
            ...
            <div className="p-6 space-y-3">
              <button onClick={() => finalizeClassification('KB3')} className="w-full p-4 bg-emerald-50 hover:bg-emerald-100 border-2 border-emerald-200 rounded-2xl flex items-center gap-4 transition-all group active:scale-[0.98]">
                <div className="w-10 h-10 bg-emerald-600 text-white rounded-xl flex items-center justify-center shadow-lg group-hover:rotate-6 transition-transform"><ScrollText className="w-5 h-5" /></div>
                <div className="text-left"><span className="block font-black text-emerald-700 text-xs uppercase tracking-tight">Văn bản Chỉ thị, Nghị quyết</span></div>
              </button>
              <button onClick={() => finalizeClassification('KB1')} className="w-full p-4 bg-red-50 hover:bg-red-100 border-2 border-red-200 rounded-2xl flex items-center gap-4 transition-all group active:scale-[0.98]">
                <div className="w-10 h-10 bg-[#B30000] text-white rounded-xl flex items-center justify-center shadow-lg group-hover:rotate-6 transition-transform"><Database className="w-5 h-5" /></div>
                <div className="text-left"><span className="block font-black text-[#B30000] text-xs uppercase tracking-tight">Tổ chức - Xây dựng Đảng</span></div>
              </button>
              <button onClick={() => finalizeClassification('KB2')} className="w-full p-4 bg-blue-50 hover:bg-blue-100 border-2 border-blue-200 rounded-2xl flex items-center gap-4 transition-all group active:scale-[0.98]">
                <div className="w-10 h-10 bg-blue-900 text-white rounded-xl flex items-center justify-center shadow-lg group-hover:rotate-6 transition-transform"><Users className="w-5 h-5" /></div>
                <div className="text-left"><span className="block font-black text-blue-900 text-xs uppercase tracking-tight">Tuyên giáo - Dân vận</span></div>
              </button>
      ...
      <header className="bg-[#B30000] text-white py-4 px-6 md:px-10 shadow-2xl border-b-2 border-[#FFD700] flex items-center justify-between sticky top-0 z-50 w-full">
        <div className="flex items-center gap-4">
          ...
          <div className="flex flex-col">
            <h1 className="text-sm md:text-xl font-black tracking-tight uppercase leading-none">Nghiệp vụ Xây dựng Đảng</h1>
            <span className="text-[10px] md:text-xs text-[#FFD700] font-black italic opacity-90 uppercase tracking-tighter">Phường Nam Hồng Lĩnh</span>
          </div>
        </div>
        <div className="flex gap-2 items-center">
          <button 
            onClick={() => setIsPro(!isPro)} 
            className={`flex items-center gap-2 px-3 py-2 md:px-4 md:py-2.5 rounded-2xl font-black text-[10px] uppercase transition-all shadow-xl active:scale-95 border-b-4 ${isPro ? 'bg-amber-500 text-white border-amber-800' : 'bg-slate-100/10 text-white/70 border-white/10 hover:bg-white/20'}`}
          >
          ...
          <button 
            onClick={() => { setShowDocClassification(true); setShowKnowledgeBase(false); }} 
            className="flex items-center gap-2 bg-white/10 text-white px-3 py-2 md:px-4 md:py-2.5 rounded-2xl font-black text-[10px] uppercase hover:bg-white/20 transition-all border-b-4 border-white/10 shadow-xl active:scale-95"
          >
            <ListFilter className="w-4 h-4 text-[#FFD700]" />
            <span className="hidden lg:inline">Phân loại văn bản</span>
          </button>

          <button 
            onClick={() => { setShowKnowledgeBase(true); setShowDocClassification(false); }} 
            className="flex items-center gap-2 bg-[#FFD700] text-[#B30000] px-3 py-2 md:px-4 md:py-2.5 rounded-2xl font-black text-[10px] uppercase hover:brightness-110 transition-all border-b-4 border-red-800/40 shadow-xl active:scale-95"
          >
      ...
      {/* Right Drawer: Doc Classification */}
      <div className={`fixed inset-y-0 right-0 w-85 md:w-[520px] bg-white shadow-[-20px_0_50px_rgba(0,0,0,0.1)] z-[110] transform transition-transform duration-500 ease-out border-l border-slate-100 flex flex-col ${showDocClassification ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className="p-6 bg-slate-900 text-white flex items-center justify-between">
          <h2 className="font-black uppercase flex items-center gap-3 text-sm tracking-tighter italic">
            <BookmarkCheck className="w-5 h-5 text-[#FFD700]" /> Phân loại & Tra cứu văn bản
          </h2>
          <button onClick={() => setShowDocClassification(false)} className="hover:bg-slate-800 p-2.5 rounded-2xl transition-all active:scale-90"><X className="w-6 h-6" /></button>
        </div>
        
        <div className="p-6 border-b border-slate-100 bg-slate-50 space-y-4">
          <div className="relative group">
            <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-400 group-focus-within:text-[#B30000] transition-colors" />
            <input 
              type="text" 
              placeholder="Tìm kiếm tên văn bản, số hiệu..."
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full pl-12 pr-4 py-4 rounded-2xl border-2 border-slate-200 focus:border-[#B30000] focus:ring-4 focus:ring-red-50 focus:outline-none font-bold text-sm transition-all"
            />
          </div>
          ...
          <div className="flex flex-wrap gap-2">
            {DOC_TYPES.map(type => (
              <button
                ...
                className={`px-4 py-2 rounded-full text-[10px] font-black uppercase tracking-tight transition-all border-2 ${selectedType === type.value ? 'bg-[#B30000] border-[#B30000] text-white shadow-lg' : 'bg-white border-slate-200 text-slate-500 hover:border-slate-300'}`}
              >
                {type.label}
              </button>
            ))}
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-4 scrollbar-hide">
          {filteredDocuments.length > 0 ? (
            filteredDocuments.map((doc, idx) => (
              <div 
                key={idx} 
                className="p-5 bg-white border border-slate-100 rounded-[28px] shadow-sm hover:shadow-xl hover:border-red-100 transition-all group flex items-start gap-4 animate-fadeIn"
                style={{ animationDelay: `${idx * 0.05}s` }}
              >
                <div className={`mt-1 p-2 rounded-xl flex items-center justify-center shrink-0 ${doc.name.includes('Chỉ thị') ? 'bg-red-50 text-red-600' : doc.name.includes('Nghị quyết') ? 'bg-emerald-50 text-emerald-600' : doc.name.includes('Kết luận') ? 'bg-blue-50 text-blue-600' : 'bg-amber-50 text-amber-600'}`}>
                  <FileText className="w-5 h-5" />
                </div>
                <div className="flex-1 space-y-2">
                  <h4 className="text-xs font-black text-slate-900 uppercase leading-snug tracking-tight group-hover:text-[#B30000] transition-colors">
                    {doc.name}
                  </h4>
                  <div className="flex items-center gap-2">
                    <span className="px-2 py-0.5 bg-slate-100 text-[9px] font-black text-slate-500 rounded-md uppercase tracking-tighter">
                      {doc.category === 'KB3' ? 'Văn bản TW' : doc.category === 'KB1' ? 'Tổ chức' : 'Tuyên giáo'}
                    </span>
                    <span className="text-[9px] text-slate-400 font-bold italic uppercase tracking-tighter">Cập nhật 2025</span>
                  </div>
                </div>
                <button 
                  onClick={() => { handleSend(`Thông tin chi tiết về văn bản: ${doc.name}`); setShowDocClassification(false); }}
                  className="mt-1 p-2 bg-slate-50 text-slate-400 rounded-xl hover:bg-[#B30000] hover:text-white transition-all active:scale-90"
                >
            ...
            <div className="flex flex-col items-center justify-center py-20 text-slate-300 gap-4">
              ...
              <p className="font-black uppercase text-xs tracking-widest">Không tìm thấy văn bản phù hợp</p>
            </div>
          )}
        </div>
      </div>
      ...
      <div className={`fixed inset-y-0 right-0 w-80 md:w-[480px] bg-white shadow-[-20px_0_50px_rgba(0,0,0,0.1)] z-[100] transform transition-transform duration-500 ease-out border-l border-slate-100 flex flex-col ${showKnowledgeBase ? 'translate-x-0' : 'translate-x-full'}`}>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-10 scrollbar-hide">
          {[KNOWLEDGE_BASE_3, KNOWLEDGE_BASE_1, KNOWLEDGE_BASE_2].map((kb, i) => (
            <section key={kb.id} className="space-y-5">
              <div className="flex items-center gap-3 border-b-2 border-slate-100 pb-2">
                <div className={`w-2 h-2 rounded-full ${kb.id === 'KB3' ? 'bg-emerald-500' : kb.id === 'KB1' ? 'bg-red-600' : 'bg-blue-600'}`}></div>
              ...
              <div className="grid gap-3">
                {kb.docs.map((doc, idx) => (
                  <div key={idx} className="flex items-start gap-4 p-4 bg-slate-50 rounded-[20px] border border-slate-200/50 hover:bg-white hover:shadow-md transition-all group">
                    <div className={`w-1 h-full min-h-[20px] rounded-full transition-all group-hover:w-2 ${kb.id === 'KB3' ? 'bg-emerald-500' : kb.id === 'KB1' ? 'bg-red-500' : 'bg-blue-500'}`}></div>
                    <span className="text-[10px] font-bold text-slate-600 leading-tight uppercase italic">{doc}</span>
                  </div>
                ))}
      ...
      {(showKnowledgeBase || showDocClassification) && (
        <div className="fixed inset-0 bg-slate-900/20 backdrop-blur-[2px] z-[90] transition-opacity duration-500" onClick={() => { setShowKnowledgeBase(false); setShowDocClassification(false); }} />
      )}
            ...
            <div className="bg-white p-12 md:p-20 rounded-[80px] shadow-[0_50px_150px_rgba(0,0,0,0.06)] border border-slate-100 w-full max-w-7xl relative">
              ...
              <div className="flex flex-col lg:flex-row gap-16 items-center">
                ...
                <div className="flex-1 w-full grid grid-cols-1 md:grid-cols-2 gap-5">
                  {[
                    "Chỉ thị 35-CT/TW về đại hội đảng các cấp?",
                    "Nghị quyết Trung ương về xây dựng, chỉnh đốn Đảng?",
                    "Nghiệm vụ tổ chức bộ máy theo KL 228-KL/TW?",
        ...
        <div className="w-full max-w-[1600px] mx-auto">
          <div className="relative group shadow-[0_20px_50px_rgba(0,0,0,0.08)] rounded-[40px]">
            <textarea 
              ...
              placeholder="Nhập câu hỏi nghiệp vụ, văn bản chỉ thị, nghị quyết cần giải đáp..." 
